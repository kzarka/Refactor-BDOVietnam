...
<div class="index-loop-wrapper">
    ...
    <div class="index-loop-wrapper-inner">
        ...
        <!-- BEGIN CATEGORY -->
        <div class="showing">
            <span>{{ (is_array($results)) ? '0' : $results->total() }} kết quả cho</span> {{ app('request')->input('q') }}
        </div>
        <!-- END CATEGORY -->
        ...
        <!-- END LOOP  -->
        @empty
        <p style="margin: 25px 10px; color: #E1E1E7; font-size: 14px">Không có kết quả nào, từ khóa tối thiểu 3 ký tự.</p>
        @endforelse

        <!-- BEGIN INCLUDE PAGINATION -->
        @if($results)
        {{ $results->appends(request()->input())->links('partials.paginator') }}
        @endif
